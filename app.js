...
function finishCelebrationShow(overlay, actionButton){
  ...
  overlay.classList.remove("celebrationMinuteShow", "celebrationEncore");
  actionButton.classList.remove("celebrating");
  actionButton.textContent = "Tekrar Kutla ❤️";
}

function restartCelebrationEncore(overlay){
  overlay.classList.remove("celebrationEncore");
  // Reflow ile kısa parıltı animasyonu güvenli biçimde yeniden başlatılır.
  void overlay.offsetWidth;
  overlay.classList.add("celebrationEncore");
  window.setTimeout(() => overlay.classList.remove("celebrationEncore"), 1800);
}
...
function startMinuteCelebrationShow(overlay, actionButton){
  ...
  if (state.running){
    restartCelebrationEncore(overlay);
    createMusicNotes(overlay, { count:8 });
    return;
  }
  ...
  const updateCountdown = () => {
    ...
    actionButton.textContent = `Gösteri sürüyor ❤️ ${remaining} sn`;
  };
  ...
  state.intervals.push(window.setInterval(() => {
    createMusicNotes(overlay, { count:5 });
  }, 4200));
...
function showCelebrationIfToday(events){
  ...
  artwork.className = "celebrationArtwork";
  ...
  artwork.addEventListener("error", () => {
    // Görsel yüklenemese bile kullanıcıya hata göstermeyip yumuşak arka planla devam eder.
    artwork.remove();
    overlay.classList.add("celebrationArtworkFallback");
  }, { once:true });

  const lightLayer = document.createElement("div");
  lightLayer.className = "celebrationLightLayer";
  ...
  close.textContent = "Gösteriyi Başlat ❤️";
  ...
  close.addEventListener("click", async (e) => {
    ...
    if (!celebrationStarted){
      ...
      try{
        ...
        // Mobil tarayıcı müziği engellerse uygulamada hata uyarısı gösterilmez.
